...
if [ "$sdl3_system" = 1 ]; then
    pm_message "Using system SDL3. Set GTAVC_SDL3=bundled to use the SDL2 shim."
    if [ -n "${GTAVC_SDL_VIDEO_DRIVER:-}" ]; then
        export SDL_VIDEODRIVER="$GTAVC_SDL_VIDEO_DRIVER"
    else
