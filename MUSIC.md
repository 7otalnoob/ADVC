...
- Download [ffmpeg_convert_gtavc.bat](https://raw.githubusercontent.com/TheOfficialFloW/gtavc_vita/master/scripts/ffmpeg_convert_gtavc.bat) (rightclick -> Save as) and copy it to your SAAT target directory.
...
- Convert them with the ffmpeg conversion script, by copying both the script and ffmpegs exe files into the SAAT directory, and double clicking the ffmpeg_convert_gtavc.bat file, or running it in the open console window.
...
- Finally, after creating the `.osw.idx` files for each one of them, just copy all of the `.osw` with their `.osw.idx` files to the `STREAMS` folder on your vita in `/switch/gtavc/audio/STREAMS` and overwrite if asked.