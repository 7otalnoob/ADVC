...
static void check_data(void) {
  ...
  if (stat("data", &st) < 0 && stat("texdb", &st) < 0 &&
    ...
    debugPrintf("WARNING: no data/ texdb/ models/ audio/ dir found -- "
                "did you extract the game data to the gtavc folder?\n");
}
...
// NOT a raw 0-15 keycode. Correct IDs (from the mobile engine / gtavc_vita config.h,
...
int main(void) {
  ...
  mkdir("/switch/gtavc/shadercache", 0777);
  setenv("MESA_SHADER_CACHE_DIR", "/switch/gtavc/shadercache", 1);
