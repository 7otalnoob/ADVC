...
static void controls_defaults(void) {
  ...
  // PSV layout (gtavc_vita/gamefiles/controls.txt), joystick-mapped actions only:
...
void patch_game(void) {
  ...
  // TODO(VC): CAutomobile::HydraulicControl exists in libGame.so (symbol
  // resolves), but the 0x4cc/0x4dc/0xb50 byte offsets below were measured on
  // SA's compiled function body and will almost certainly point at different
  // instructions in VC's build -- re-derive them in Ghidra before enabling.
  if (0 && so_try_find_addr_rx(&game_mod, "_ZN11CAutomobile16HydraulicControlEv")) {
  ...
  if (!config.mobile_widgets) {
    // TODO(VC): CHud::DrawAfterFade resolves in libGame.so, but 0x164 was
    // measured on SA's build -- re-derive in Ghidra before enabling.
    if (0 && so_try_find_addr_rx(&game_mod, "_ZN4CHud13DrawAfterFadeEv")) {
    ...
    // TODO(VC): CPad::Update resolves in libGame.so, but 0x584 was measured
    // on SA's build -- re-derive in Ghidra before enabling.
    if (0 && so_try_find_addr_rx(&game_mod, "_ZN4CPad6UpdateEs")) {
  ...
  if (so_try_find_addr_rx(&game_mod, "_ZN5CDraw6SetFOVEfb")) {
    ...
    // half 2
    // TODO(VC): CCamera::Process resolves in libGame.so, but 0xDC8/0xDB8 were
    // measured on SA's build -- re-derive in Ghidra before enabling. Also
    // note this whole block only runs once CDraw::SetFOV's VC signature
    // (currently a signature mismatch, see near-match report) is fixed above.
    if (0 && so_try_find_addr_rx(&game_mod, "_ZN7CCamera7ProcessEv")) {
      ccamera_fov_ret =
  ...
  // TODO(VC): CCoronas::Render resolves in libGame.so, but 0x414/0x404 were
  // measured on SA's build -- re-derive in Ghidra before enabling (only
  // reachable if config.ps2_corona_rotation is turned on).
  if (0 && config.ps2_corona_rotation &&
  ...
  // TODO(VC): CCarCtrl::PossiblyRemoveVehicle resolves in libGame.so, but
  // 0x10c was measured on SA's build -- re-derive in Ghidra before enabling
  // (only reachable if config.no_offscreen_despawn is turned on).
  if (0 && config.no_offscreen_despawn &&
  ...
  // Both symbols resolve in libGame.so. Unlike the byte-offset patches
  // elsewhere in this file, this uses a full-function hook (hook_arm64
  // overwrites the whole prologue) with a generic 16-byte trampoline
  // continuation -- lower risk than a hand-measured offset, but the
  // "+16 = past the prologue" assumption still depends on VC's actual
  // codegen for this function. Verify on-device (does the taxi light still
  // toggle / does the game crash when a taxi is on screen) before trusting it.
  ...
  // TODO(VC): CAutomobile::PreRender resolves in libGame.so, but 0x864 was
  // measured on SA's build -- re-derive in Ghidra before enabling.
  if (0 && so_try_find_addr_rx(&game_mod, "_ZN11CAutomobile9PreRenderEv")) {
    uint32_t *fn =
